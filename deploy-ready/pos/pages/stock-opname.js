...
function renderStockOpname() {
  ...
  return `
    <section class="stock-page stock-opname-page">
      ...
      ${tab === "inventory" ? renderStockInventoryPanel() : tab === "riwayat" ? renderStockHistoryPanel() : renderStockAuditPanel()}
    </section>
  `;
}
...
function renderStockAuditPanel() {
  ...
  return `
    ...
    <div class="stock-list-card">
      ...
      ${filtered.length ? `<button class="btn accent stock-audit-submit" type="button" onclick="submitStockAudit()">Submit Audit</button>` : ""}
    </div>
  `;
}
    ...
    <article class="stock-opname-item ${checked ? "checked" : ""}" data-stock-row data-stock-search="${escapeHtml(search)}">
      ...
      <div class="stock-opname-info">
        ...
        <div class="stock-audit-entry" data-audit-product-id="${escapeHtml(product.id)}">
          ...
          <label><span>Stok saat ini</span><input inputmode="decimal" type="number" min="0" step="1" value="0" data-dirty="0" oninput="this.dataset.dirty='1'" aria-label="Stok saat ini ${escapeHtml(product.name)}" /></label>
        </div>
      </div>
    </article>
  `;
...
function submitStockAudit() {
  ...
  if (!checkedCount) return toast("Isi stok nyata minimal satu barang.");
  audit("Audit stok opname", `${checkedCount} barang dicek, ${adjustedCount} disesuaikan`);
  saveState();
  syncProductsByIds(changedProductIds);
  saveStockAuditCheckedIds([]);
  render();
  toast("Audit stok tersimpan.");
}

...
async function submitStockAudit() {
  ...
  try {
    ...
    toast(stockAuditResultMessage(checkedCount, adjustedCount, syncResults.every(Boolean)), syncResults.every(Boolean) ? 3600 : 4800);
  } catch (error) {
    console.error("Submit stock audit failed", error);
    toast(`Gagal submit audit: ${error.message || "Coba lagi."}`, 4200);
  } finally {
    stockAuditSubmitting = false;
    if (stopLoading) stopLoading();
    render();

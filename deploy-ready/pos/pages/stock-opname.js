...
function stockAuditResultMessage(checkedCount, adjustedCount, synced) {
  const summary = `${checkedCount} item dicek, stok ${adjustedCount} item diupdate`;
  return synced
    ? `Audit stok berhasil: ${summary}.`
    : `Audit stok tersimpan lokal: ${summary}. Sync Supabase masuk antrean.`;
}
...
function stockAuditSubmitButtonHtml(hasRows) {
  ...
  if (stockAuditSubmitStatus?.message) {
    return `<button class="btn stock-audit-submit is-status ${escapeHtml(stockAuditSubmitStatus.type || "info")}" type="button" disabled aria-live="polite">${escapeHtml(stockAuditSubmitStatus.message)}</button>`;
  }
...
async function submitStockAudit() {
  ...
  try {
    ...
    const synced = syncResults.every(Boolean);
    stockAuditSubmitStatus = { type: synced ? "success" : "pending", message: stockAuditResultMessage(checkedCount, adjustedCount, synced) };
  } catch (error) {

...
function renderSelfOrderUpsellSettings(activeProducts = []) {
  ...
  const selectedProducts = selectedIds
    ...
    .filter(Boolean);
  const selectableProducts = activeProducts.filter(product => !selectedIds.includes(product.id));
  return `
    <section class="self-order-upsell-settings">
      <div class="self-order-upsell-settings-head">
        <div>
          <strong>Produk rekomendasi</strong>
          <span>Pilih 4-8 produk yang akan tampil acak di Keranjang dan Pembayaran.</span>
        </div>
        <small>${selectedProducts.length}/8 produk</small>
      </div>
      <div class="self-order-upsell-picker">
        <select data-self-order-upsell-select ${selectedIds.length >= 8 || !selectableProducts.length ? "disabled" : ""}>
          <option value="">Pilih produk</option>
          ${selectableProducts.map(product => `<option value="${escapeHtml(product.id)}">${escapeHtml(product.name)} - ${productDisplayPrice(product)}</option>`).join("")}
        </select>
        <button class="btn" type="button" onclick="addSelfOrderUpsellProductFromSelect(this.closest('.self-order-upsell-settings'))" ${selectedIds.length >= 8 || !selectableProducts.length ? "disabled" : ""}>Tambah</button>
      </div>
      <div class="self-order-upsell-settings-grid">
        ${selectedProducts.map(product => `
          <article class="self-order-upsell-settings-card">
            <span>${productImageOrToken(product, "self-order-upsell-settings-img", 160)}</span>
            <div>
              <strong>${escapeHtml(product.name)}</strong>
              <small>${escapeHtml(product.category || "Tanpa kategori")} - ${productDisplayPrice(product)}</small>
            </div>
            <button type="button" onclick="removeSelfOrderUpsellProduct('${escapeHtml(product.id)}')" aria-label="Hapus ${escapeHtml(product.name)}">&times;</button>
            <input type="hidden" name="selfOrderUpsellProductIds" value="${escapeHtml(product.id)}" />
          </article>
...
function addSelfOrderUpsellProductFromSelect(container) {
  const select = container?.querySelector?.("[data-self-order-upsell-select]");
  const productId = String(select?.value || "").trim();
  if (!productId) return toast("Pilih produk rekomendasi terlebih dahulu.");
  const ids = normalizeSelfOrderUpsellProductIds(state.settings.selfOrderUpsellProductIds || []);
  if (ids.length >= 8) return toast("Maksimal 8 produk rekomendasi.");
  if (ids.includes(productId)) return toast("Produk sudah ada di rekomendasi.");
  state.settings.selfOrderUpsellProductIds = normalizeSelfOrderUpsellProductIds([...ids, productId]);
  render();
}

...
function renderSelfOrderUpsellSettings(activeProducts = []) {
  ...
  const selectableEntries = allEntries
    .filter(entry => !selectedIds.includes(entry.key))
    .filter(entry => activeCategory === "Semua" || entry.category === activeCategory)
    .filter(entry => !query || [entry.name, entry.product.name, entry.category].join(" ").toLowerCase().includes(query));
  return `
    <section class="self-order-upsell-settings">
      <div class="self-order-upsell-settings-head">
        ...
        <small>${selectedEntries.length}/8 produk</small>
      </div>
      ...
      <div class="self-order-upsell-picker">
        ${selectableEntries.map(entry => `
          <article class="self-order-upsell-pick-card" data-upsell-search-text="${escapeHtml([entry.name, entry.product.name, entry.category].join(" ").toLowerCase())}">
            ...
            <button type="button" onclick="addSelfOrderUpsellSettingEntry('${escapeHtml(entry.key)}')" ${selectedIds.length >= 8 ? "disabled" : ""} aria-label="Tambah ${escapeHtml(entry.name)}">+</button>
          </article>
      ...
      <div class="self-order-upsell-settings-grid">
        ${selectedEntries.map(entry => `
          <article class="self-order-upsell-settings-card">
            ...
            <button type="button" onclick="removeSelfOrderUpsellProduct('${escapeHtml(entry.key)}')" aria-label="Hapus ${escapeHtml(entry.name)}">&times;</button>

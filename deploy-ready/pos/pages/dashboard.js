...
function renderDashboard() {
  ...
  const cancelledValue = cancelled.reduce((sum, order) => sum + orderTotal(order), 0);
  ...
  return `
    ...
    <div class="dashboard-main-grid">
      ...
      <div class="dashboard-side-stack">
        ...
        <button class="card dashboard-cancel-card" type="button" onclick="openCancelledOrdersReport()">
          <div>
            <span class="dashboard-cancel-icon">${navIcon("cancel")}</span>
            <strong>Pesanan Dibatalkan</strong>
            <small>${cancelled.length ? `${cancelled.length} pesanan pada periode ini` : "Tidak ada pembatalan"}</small>
          </div>
          <b>${cancelled.length ? money(cancelledValue) : "Bersih"}</b>
        </button>

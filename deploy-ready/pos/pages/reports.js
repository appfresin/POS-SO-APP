...
function renderProfitLossDrill() {
  ...
  const yearRows = profitSummaryCache.years;
  ...
  if (level === "today") {
    ...
    backButton = "";
    buckets = (profitSummaryCache.days[`${thisYear}-${thisMonth + 1}`] || []).map(row => ({
    ...
    backButton = "";
    buckets = (profitSummaryCache.months[thisYear] || []).map(row => ({
      id: String(row.month),
    ...
    buckets = (profitSummaryCache.months[selectedYear] || []).map(row => ({
      id: String(row.month),
    ...
    buckets = (profitSummaryCache.days[`${selectedYear}-${selectedMonth}`] || []).map(row => ({
  ...
  const listRows = displayRows.map(bucket => {
    ...
    return `
      <button type="button" class="profit-drill-row ${clickable ? "clickable" : ""}" ${clickable ? `onclick="${escapeHtml(bucket.action)}"` : ""}>
  ...
  const monthRowsForCurrentYear = profitSummaryCache.months[thisYear] || [];
  ...
  const dayRowsForCurrentMonth = profitSummaryCache.days[dayKey] || [];

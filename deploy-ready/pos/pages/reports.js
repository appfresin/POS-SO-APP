...
function reportReceiptMenu(order) {
  ...
  return `
    <details class="visitor-receipt-menu">
      <summary aria-label="Menu struk">&vellip;</summary>
      <div class="visitor-receipt-menu-panel">
...
function renderProfitRowsWithShiftBadges(rows, range) {
  ...
  ["shift1", "shift2"].forEach(shiftKey => {
    if (!sessions[shiftKey].length && !grouped[shiftKey].length) return;

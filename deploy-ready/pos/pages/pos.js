          ...
          <div class="cart-payment-page">
            <div class="payment-detail-shell">
              ...
              <div class="payment-detail-fields">
                ...
                <div class="field" data-mobile-label="Bayar"><label>Pembayaran</label><select id="paymentMethod" onchange="sessionStorage.setItem('pos_payment_method', this.value); refreshPosOrRender({ preserveScroll: true })">${["Tunai", "QRIS", "Transfer", "Kartu Debit", "E-wallet", "Pembayaran Delivery"].map(method => `<option ${method === paymentMethod ? "selected" : ""}>${method}</option>`).join("")}</select></div>
                ...
                <div class="field discount-field" data-mobile-label="Diskon"><label>Diskon</label><div class="discount-inline"><input id="pos_discount" class="input" type="number" min="0" max="${discountType === "percent" ? "100" : ""}" value="${discount}" oninput="sessionStorage.setItem('pos_discount', this.value)" onchange="refreshPosOrRender({ preserveScroll: true })" /><div class="discount-type-tabs" role="group" aria-label="Jenis diskon transaksi"><button type="button" class="${discountType === "rp" ? "active" : ""}" onclick="setOrderDiscountType('rp')">Rp</button><button type="button" class="${discountType === "percent" ? "active" : ""}" onclick="setOrderDiscountType('percent')">%</button></div></div></div>
              </div>
            </div>

...
async function registerNativePushToken(options = {}) {
  ...
  const registerKey = `${session.user.id}|${scopeKey}|${target}|${token}`;
...
function normalizeStoreSettingsRow(row) {
  ...
  return {
    ...
    receiptLogoPrintDataUrl: receiptSettings.logoPrintDataUrl || state.settings.receiptLogoPrintDataUrl || "",
  ...
  return {
    ...
    self_order_settings: {
      ...
      receipt: {
        ...
        logoPrintDataUrl: settings.receiptLogoPrintDataUrl || "",
...
function receiptPrintWidth(settings = state.settings || {}) {
  return settings.receiptWidth === "80mm" ? 42 : 32;
}
...
function receiptPrintModel(order = null) {
  ...
  return {
    ...
    logo: {
      ...
      dataUrl: settings.receiptLogoPrintDataUrl || settings.receiptLogoDataUrl || "",
    ...
    totalLines: [
      ...
      ...(source.receivedAmount ? [{ label: "Bayar", value: receiptMoneyText(source.receivedAmount) }] : []),
      ...(source.changeAmount ? [{ label: "Kembali", value: receiptMoneyText(source.changeAmount) }] : [])
    ],
...
function receiptPairText(label, value, width) {
  ...
  const lines = receiptWrapText(left, available);
...
function selfOrderAddProduct(productId) {
  ...
  selfOrderUpsertDetailCartItem(product, detailState);
  sessionStorage.setItem("self_order_step", "menu");
  sessionStorage.removeItem("self_order_product_id");
  sessionStorage.setItem("self_order_detail_qty", "1");
  render();
...
function createReceiptLogoPrintDataUrl(file) {
...
async function uploadReceiptLogo(input) {
  ...
  const previousPrintLogo = state.settings.receiptLogoPrintDataUrl || "";
  ...
  try {
    ...
    state.settings.receiptLogoPrintDataUrl = await createReceiptLogoPrintDataUrl(file);
    ...
    state.settings.receiptLogoPrintDataUrl = previousPrintLogo;

...
const ROLE_DEFINITIONS = {
  ...
  manager: {
    label: "Manager",
    views: ["dashboard", "orders", "selforder", "products", "stock", "stock-opname", "reports", "settings"]
  },
  kasir: {
    label: "Kasir",
    views: ["pos", "orders", "selforder", "settings"]
  },
  dapur: {
    label: "Dapur",
...
function normalizeStaffState() {
  ...
  Object.entries(defaultAccess).forEach(([role, views]) => {
    ...
    if (views.includes("settings") && !filtered.includes("settings")) filtered.push("settings");
  ...
  state.staff.members = mergedMembers.map(member => ({
    ...
    menuAccess: (() => {
      ...
      if ((defaultAccess[member.role] || []).includes("settings") && !access.includes("settings")) access.push("settings");
      return access;
    })()
  }));
...
function renderOrders() {
  ...
  const visibleCount = filtered.filter(order => orderMatchesSearch(order, query)).length;
  const statCards = [
  ...
  return `
    <section class="orders-page">
      ...
      <div class="orders-list-grid" data-orders-list>
        ${filtered.map(order => orderCenterCard(order, { searchQuery: query })).join("")}

...
function selfOrderApplyDetailQty(product, detailState, nextQty) {
  ...
  if (Number.isFinite(limit) && limitedStockCartQty(selfOrderCart, product.id, variantKey, currentLineId) + nextQty > limit) {
    return toast("Sisa stok habis");
  }
  if (detailState.line) {
    if (nextQty <= 0) {
      selfOrderRemoveDetailCartLine(detailState.line);
      return selfOrderCancelDetailSelection(product, `${product.name} dihapus dari keranjang.`);
    }
    ...
    return toast(`Jumlah ${product.name} menjadi ${nextQty}.`);
  }
  sessionStorage.setItem("self_order_detail_qty", String(nextQty));
  selfOrderRefreshDetailQtyLabel(nextQty);
  selfOrderUpdateAddButton(product.id);
  if (nextQty <= 0) selfOrderCancelDetailSelection(product, `${product.name} tidak jadi dipilih.`);
}

function selfOrderSetDetailQty(productId, delta) {
  const product = selfOrderProduct(productId);
  if (!product) return;
  const detailState = selfOrderDetailCartState(product);
  ...
  selfOrderApplyDetailQty(product, detailState, nextQty);
}
...
function selfOrderUpsertDetailCartItem(product, detailState) {
...
function selfOrderAddProduct(productId) {
  ...
  if (!selfOrderUpsertDetailCartItem(product, detailState)) return;
  sessionStorage.setItem("self_order_step", "menu");
  sessionStorage.removeItem("self_order_product_id");
  sessionStorage.setItem("self_order_detail_qty", "1");
  render();
...
async function selfOrderCreateOrder() {
  ...
  if (!stockValidation.ok) {
    selfOrderSubmitError = "Sisa stok habis";
    return;
  }
  [...new Set(orderItems.map(item => item.productId))].forEach(productId => {
    const product = state.products.find(candidate => candidate.id === productId);
    if (product) stockRollback.push({ productId, snapshot: JSON.stringify(product) });
  });
  touchedProductIds.push(...applyLimitedStockItems(orderItems, -1, `Self order ${order.number}`));
  if (openOrder) {
  ...
  if (!synced) {
    stockRollback.forEach(entry => {
      const index = state.products.findIndex(product => product.id === entry.productId);
      if (index >= 0) state.products[index] = JSON.parse(entry.snapshot);
    });
...
function addToCart(id) {
  ...
  if (productSaleStockLimit(product) <= 0) return toast("Sisa stok habis");
  const existing = cart.find(item => item.productId === id);
  const qty = existing && !hasVariants ? existing.qty + 1 : 1;
  if (limitedStockCartQty(cart, id) + 1 > productSaleStockLimit(product)) return toast("Sisa stok habis");
...
function submitOrder() {
  ...
  if (!stockValidation.ok) return toast("Sisa stok habis");

...
function hasPendingMasterUpserts() {
  return (state.syncQueue || []).some(item => item.type === "master" && item.operation !== "delete");
}
...
async function loadMasterDataFromSupabase(options = {}) {
  ...
  try {
    ...
    const remoteMasterIsEmpty = !allServerProducts.length && !serverAddons.length && !serverCategories.length;
    if (remoteMasterIsEmpty && (state.products.length || state.addons.length || state.categories.length) && !hasPendingMasterUpserts()) {
    ...
    const localProductsMissingOnServer = state.products.filter(local => !isDeletedProduct(local) && !serverProducts.some(server => server.id === local.id));

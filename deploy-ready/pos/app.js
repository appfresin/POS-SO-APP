...
const ROLE_DEFINITIONS = {
  owner: {
    label: "Owner",
    views: ["dashboard", "pos", "kitchen", "orders", "selforder", "products", "reports", "settings"]
  },
  manager: {
    label: "Manager",
    views: ["dashboard", "orders", "selforder", "products", "reports"]
  },
  kasir: {
    label: "Kasir",
...
const DEFAULT_STAFF_MEMBERS = [
  { id: "owner", name: "Owner", role: "owner", pin: "0000", active: true },
  { id: "manager", name: "Manager", role: "manager", pin: "2222", active: true },
  { id: "kasir", name: "Kasir", role: "kasir", pin: "1111", active: true },
  { id: "dapur", name: "Dapur", role: "dapur", pin: "3333", active: true }
];
...
const initialState = {
  ...
  products: [
    { id: uid(), name: "Es Kopi Susu", sku: "KOP-001", category: "Kopi", price: 18000, cost: 9000, stock: 45, minStock: 10, active: true, soldOut: false, trackStock: true, prepMinutes: 8 },
    { id: uid(), name: "Butter Croissant", sku: "FOD-002", category: "Makanan", price: 24000, cost: 12000, stock: 12, minStock: 8, active: true, soldOut: false, trackStock: true, prepMinutes: 12 },
    { id: uid(), name: "Iced Matcha", sku: "DRK-003", category: "Non-Kopi", price: 22000, cost: 10000, stock: 30, minStock: 10, active: true, soldOut: false, trackStock: true, prepMinutes: 7 },
    { id: uid(), name: "Avocado Toast", sku: "FOD-004", category: "Makanan", price: 45000, cost: 23000, stock: 0, minStock: 5, active: true, soldOut: true, trackStock: true, prepMinutes: 15 },
    { id: uid(), name: "Air Mineral", sku: "DRK-006", category: "Minuman", price: 8000, cost: 3500, stock: 80, minStock: 20, active: true, soldOut: false, trackStock: true, prepMinutes: 1 }
  ],
  addons: [
    { id: uid(), name: "Extra Shot", price: 6000, cost: 2500, active: true },
    { id: uid(), name: "Jelly Topping", price: 2000, cost: 800, active: true },
    { id: uid(), name: "Salt Cream", price: 5000, cost: 2200, active: true }
  ],
  categories: ["Kopi", "Makanan", "Non-Kopi", "Minuman"],
  settings: {
  ...
  heldOrders: [],
  audit: [
    { id: uid(), at: new Date().toISOString(), user: "System", action: "Aplikasi diinisialisasi", detail: "Seed data produk dan sesi kas dibuat" }
  ],
  stockMovements: []
};
...
function normalizeState() {
  ...
  state.addons = state.addons.map(addon => ({ active: true, cost: 0, ...addon, price: Number(addon.price || 0), cost: Number(addon.cost || 0) }));
...
function stableProducts(products) {
  return [...(products || [])].sort((a, b) => productSortKey(a).localeCompare(productSortKey(b)));
}
...
function selfOrderTableCodeFromUrl() {
  ...
  try {
    ...
    candidates.push(search.get("table"), search.get("meja"));
    const hashQuery = String(location.hash || "").split("?")[1] || "";
    if (hashQuery) {
      const hashParams = new URLSearchParams(hashQuery);
      candidates.push(hashParams.get("table"), hashParams.get("meja"));
    }
  } catch (error) {
  ...
  return /^A-(?:[1-9]|1\d|20)$/.test(code) ? code : "";
}
...
function normalizeStaffState() {
  ...
  const mergedMembers = [...DEFAULT_STAFF_MEMBERS.map(member => ({ ...member }))];
  ...
  state.staff.members = mergedMembers;
  const activeStillValid = state.staff.members.some(member => member.id === state.staff.activeStaffId && member.active !== false);
  if (!activeStillValid && !state.staff.activeStaffId) return;
  if (!activeStillValid) state.staff.activeStaffId = "owner";
}
...
function allowedViews(role = activeRole()) {
  normalizeStaffState();
  if (role === "owner") return ACCESS_VIEW_OPTIONS.map(option => option.id);
  return state.staff.roleAccess?.[role] || ROLE_DEFINITIONS[role]?.views || [];
}

function defaultViewForRole(role = activeRole()) {
  return allowedViews(role)[0] || "dashboard";
}
...
function appAvailableViews() {
  const views = ["dashboard", "pos", "kitchen", "orders", "selforder", "products", "stock", "reports", "cash", "audit", "settings"];
...
function mediaImageFallbackScript(value, size = 160) {
  ...
  return `this.dataset.mediaIndex=(Number(this.dataset.mediaIndex||0)+1);var media=${escapeHtml(JSON.stringify(candidates))};if(media[this.dataset.mediaIndex]){this.src=media[this.dataset.mediaIndex];}else{this.onerror=null;this.hidden=true;}`;
}
...
function shouldRenderAfterRealtimeLoad(kind) {
  if (view !== "selforder") return ["pos", "kitchen", "orders", "dashboard", "products"].includes(view);
...
async function reloadStateFromLocalPersistence() {
  try {
    ...
    normalizeState();
    if (view !== "selforder" || selfOrderStep() === "menu") render();
  } catch (error) {
    try {
...
async function ensureSupabaseProfile() {
  ...
  const payload = {
    id: user.id,
    full_name: state.activeUser || user.email || "Admin",
    role: "admin",
    active: true
  };
...
function isReportableRecord(record) {
  ...
  return record.imported || record.paymentStatus === "Lunas";
}
...
function masterQueueKey(entity, localId, operation = "upsert") {
  return `${entity}:${operation}:${localId}`;
}

function markMasterSyncPending(entity, payload, operation = "upsert", error = null) {
  state.syncQueue ||= [];
  const localId = payload.local_id || payload.id || payload.name || payload.localId;
  if (!localId) return;
  const key = masterQueueKey(entity, localId, operation);
  const existing = state.syncQueue.find(item => item.type === "master" && item.key === key);
  const attempts = Number(existing?.attempts || 0) + 1;
  const now = Date.now();
  const queueItem = {
    id: existing?.id || uid(),
...
function markMasterSynced(entity, payload, operation = "upsert") {
  const localId = payload.local_id || payload.id || payload.name || payload.localId;
  if (!localId) return;
  const key = masterQueueKey(entity, localId, operation);
  state.syncQueue = (state.syncQueue || []).filter(item => !(item.type === "master" && item.key === key));
}
...
async function upsertSupabaseAddon(addon) {
  ...
  if (existing?.id) {
    const { error } = await supabaseClient.from("addons").update(payload).eq("id", existing.id);
    if (error) throw error;
    return existing.id;
  }
  const { data, error } = await supabaseClient.from("addons").insert(payload).select("id").single();
  if (error) throw error;
  return data.id;
}
...
async function upsertSupabaseProduct(product) {
  ...
  if (productId) {
    ...
    if (error?.code === "23505" && payload.sku) {
      ...
      const { error: updateError } = await supabaseClient.from("products").update(payload).eq("id", conflict.id);
  ...
  if (addonIds.length) {
    ...
    const rows = [...new Map((addonRows || []).map(row => [row.id, { product_id: productId, addon_id: row.id }])).values()];
    if (rows.length) {
      const { error } = await supabaseClient
        .from("product_addons")
        .upsert(rows, { onConflict: "product_id,addon_id", ignoreDuplicates: true });
      if (error && !isSupabaseDuplicateKeyError(error)) throw error;
    }
  }
  return productId;
}
...
async function syncMasterQueueItem(item) {
  if (item.operation === "delete") {
    ...
    markMasterSynced(item.entity, item.payload, item.operation);
    return;
  }
  if (item.entity === "category") await ensureSupabaseCategory(item.payload.name);
  if (item.entity === "addon") await upsertSupabaseAddon(item.payload);
  if (item.entity === "product") await upsertSupabaseProduct(item.payload);
  markMasterSynced(item.entity, item.payload, item.operation);
}

async function syncMasterEntity(entity, payload, operation = "upsert") {
  if (!payload) return false;
  if (!canAttemptSupabaseSync()) {
    markMasterSyncPending(entity, payload, operation);
    saveState();
    return false;
  }
  try {
    await syncMasterQueueItem({ entity, payload, operation });
    saveState();
    return true;
  } catch (error) {
    console.error("Supabase master sync failed", entity, error);
    markMasterSyncPending(entity, payload, operation, error);
    saveState();
    return false;
  }
}
...
function syncProductsByIds(productIds) {
  [...new Set(productIds.filter(Boolean))].forEach(productId => {
    ...
    if (product) syncMasterEntity("product", product);
  });
}
...
async function processSyncQueue(reason = "manual") {
  ...
  try {
    ...
    const pendingItems = [...state.syncQueue]
      .filter(item => (item.type === "order" || item.type === "master") && (!item.nextAttemptAt || new Date(item.nextAttemptAt).getTime() <= now))
      .slice(0, 10);
    for (const item of pendingItems) {
      if (item.type === "master") {
        try {
          ...
          markMasterSyncPending(item.entity, item.payload, item.operation, error);
          saveState();
        }
        continue;
      }
...
async function upsertSupabaseOrderPayload(orderPayload) {
  const writePayload = supabaseSupportsPreparedItems
    ? orderPayload
  ...
  const { prepared_items, ...compatiblePayload } = orderPayload;
...
async function pushAllLocalMasterDataToSupabase() {
  if (!canAttemptSupabaseSync()) return false;
  const categories = productCategories();
  for (const category of categories) await ensureSupabaseCategory(category);
  for (const addon of state.addons || []) await syncMasterEntity("addon", addon);
  for (const product of state.products || []) await syncMasterEntity("product", product);
  return true;
}
...
async function loadMasterDataFromSupabase(options = {}) {
  ...
  try {
    const [categoryResult, addonResult, productResult, productAddonResult] = await Promise.all([
      ...
      supabaseClient.from("addons").select("id,local_id,name,price,cost,active,sort_order").order("name", { ascending: true }),
    ...
    const serverCategories = (categoryResult.data || []).map(normalizeSupabaseCategory).filter(Boolean);
    const serverAddons = (addonResult.data || []).map(normalizeSupabaseAddon).filter(addon => addon.name);
    ...
    if (!serverProducts.length && !serverAddons.length && (state.products.length || state.addons.length || state.categories.length)) {
      await pushAllLocalMasterDataToSupabase();
      masterSyncLoaded = true;
      masterSyncLoadedAt = Date.now();
      return;
    }

    const localProductsMissingOnServer = state.products.filter(local => !serverProducts.some(server => server.id === local.id));
    const localAddonsMissingOnServer = state.addons.filter(local => !serverAddons.some(server => server.id === local.id));
    const localCategoriesMissingOnServer = state.categories.filter(local => !serverCategories.includes(local));

    state.categories = [...new Set([...serverCategories, ...localCategoriesMissingOnServer])];
    state.addons = [
      ...serverAddons,
      ...localAddonsMissingOnServer
    ];
    state.products = [
      ...serverProducts,
      ...localProductsMissingOnServer
    ];
    normalizeState();
    saveState();

    for (const category of localCategoriesMissingOnServer) syncMasterEntity("category", categoryPayload(category));
    for (const addon of localAddonsMissingOnServer) syncMasterEntity("addon", addon);
    for (const product of localProductsMissingOnServer) syncMasterEntity("product", product);
...
function mergePreparedItems(remotePrepared = {}, localPrepared = {}) {
  return { ...(remotePrepared || {}), ...(localPrepared || {}) };
}
  ...
  try {
    ...
    for (let from = 0; ; from += pageSize) {
      ...
      const { data, error } = await supabaseClient
        ...
        .select("id,number,created_at,source,order_type,customer_name,payment_status,payment_method,subtotal,discount,total,profit,imported")
...
async function loadProfitSummary(level = "years", year = null, month = null, force = false) {
  ...
  if (level === "years" && profitSummaryCache.years.length && !force) return;
  if (level === "months" && profitSummaryCache.months[year]?.length && !force) return;
  const dayKey = `${year}-${month}`;
  if (level === "days" && profitSummaryCache.days[dayKey]?.length && !force) return;
...
function updatePosSearch(input) {
  ...
  requestAnimationFrame(() => {
    const next = document.querySelector("[data-pos-search]");
    if (!next) return;
...
function normalizeView(id) {
  ...
  return canAccessView(normalized) ? normalized : defaultViewForRole();
}
...
function topActions() {
  ...
  if (view === "stock") return `<button class="btn accent" onclick="openStockFormV2()">Penyesuaian Stok</button>`;
...
function renderAccessDenied() {
  return `
    <section class="staff-empty-state">
      ...
      <button class="btn accent" onclick="go('${defaultViewForRole()}')">Ke halaman utama</button>
    </section>
  `;
}
    ...
    <main class="staff-lock-screen">
      <section class="staff-lock-card">
        <div class="staff-lock-brand">
          <span>${staffInitials("Kasirin")}</span>
          <div>
            <h1>Kasirin!</h1>
            <p>Masuk sebagai staff untuk membuka menu kerja.</p>
          </div>
        </div>
...
function loginStaffFromForm(event) {
  event.preventDefault();
  const form = event.currentTarget;
  const staffCode = String(form.staffCode?.value || form.staffId?.value || "").trim().toLowerCase();
  const pin = String(form.pin?.value || "").trim();
  const member = staffMembers().find(item => {
    const id = String(item.id || "").toLowerCase();
    const name = String(item.name || "").toLowerCase();
    return id === staffCode || name === staffCode;
  });
  if (!member || String(member.pin || "") !== pin) {
    toast("Nama staff atau PIN belum sesuai.");
    return;
  }
  ...
  staffMenuOpen = false;
  saveState();
  const nextView = defaultViewForRole(member.role);
  view = nextView;
...
function saveStaffMember(event) {
  event.preventDefault();
  const form = event.currentTarget;
  const editId = String(form.staffEditId?.value || "").trim();
  const name = String(form.staffName?.value || "").trim();
  const role = form.staffRole?.value;
  const pin = String(form.staffPin?.value || "").trim();
  if (!name || !ROLE_DEFINITIONS[role] || pin.length < 4) {
    toast("Isi nama, role, dan PIN minimal 4 digit.");
    return;
  }
  normalizeStaffState();
  ...
  if (editingMember) {
    ...
    if (state.staff.activeStaffId === editingMember.id) state.activeUser = name;
    state.staff.editingStaffId = "";
    toast("Staff diperbarui.");
  } else {
    state.staff.members.push({ id: uid(), name, role, pin, active: true });
...
function removeStaffMember(id) {
  ...
  if (state.staff.activeStaffId === id) state.staff.activeStaffId = "owner";
  saveState();
  render();
}
...
function quickLoginStaff(id) {
  ...
  staffMenuOpen = false;
  saveState();
  go(defaultViewForRole(member.role));
}
...
function categoryCount(category) {
  if (category === "Semua") return state.products.filter(product => product.active).length;
  return state.products.filter(product => product.active && product.category === category).length;
}
...
function selfOrderAvailableAddons(product) {
  ...
  return state.addons.filter(addon => addon.active && allowedIds.includes(addon.id));
}
...
async function selfOrderCreateOrder() {
  ...
  syncOrderToSupabase(order, { silent: true }).then(synced => {
  ...
  render();
    ...
    toast(`Pesanan belum bisa diproses: ${error.message}`);
  } finally {
...
function renderSelfOrderCatalog() {
  ...
  const categories = ["Semua", ...productCategories()];
  const activeProducts = stableProducts(state.products).filter(product => product.active && !product.soldOut);
  const categoryProducts = activeProducts.filter(product => category === "Semua" || product.category === category);
  const visibleCategories = productCategories().filter(item => activeProducts.some(product => product.category === item));
  return `
    <section class="self-order-categories" aria-label="Kategori produk">
      ${categories.map(item => `
        <button class="${item === category ? "active" : ""}" type="button" onclick='selfOrderSetCategory(${JSON.stringify(item)})'>
          ${escapeHtml(item)}
        </button>
      `).join("")}
    </section>
    ${category === "Semua"
      ...
      : renderSelfOrderCategorySection(category, categoryProducts, false)}
  `;
}
...
function renderSelfOrderPromo() {
  ...
  return `
    <section class="self-order-promo ${hasImage ? "has-image" : "is-empty"}" data-active-index="${activeIndex}" style="--promo-index:${activeIndex};--promo-offset:0%">
  ...
  return `
    <main class="self-order-main detail">
      ...
      <section class="self-order-detail">
        <div class="self-order-detail-media">${productImageOrToken(product, "self-order-detail-img", 720)}</div>
...
function renderSelfOrderPayment() {
  ...
  return `
    <main class="self-order-main payment">
      ...
      <button class="self-order-primary self-order-finish" type="button" onclick="selfOrderCreateOrder()">Selesaikan Pesanan</button>
    </main>
  `;
}
...
function productCard(product) {
  ...
  const photo = mediaImageTag(product.imageName, `Foto ${product.name}`, "product-thumb", 120);
  return `
    <button class="product-card" onclick="addToCart('${product.id}')" ${disabled ? "disabled" : ""}>
      <div class="product-token ${photo ? "has-photo" : ""}">${photo || escapeHtml(product.name.slice(0, 2).toUpperCase())}</div>
...
function cartItemOptions(item, addonProducts = []) {
  ...
  const availableAddons = addonProducts.filter(addon => allowedAddonIds.includes(addon.id));
  return `
...
function addAddonToCartItem(itemKey, addonId) {
  ...
  const addonProduct = state.addons.find(addon => addon.id === addonId && addon.active);
...
function payUnpaidOrder(id) {
  ...
  location.hash = "pos";
  render();
  toast(`${order.number} dibuka untuk pembayaran.`);
}
...
function resumeHeldOrder(id) {
  ...
  toast("Pesanan tersimpan dibuka.");
}
...
function kitchenOrderItemKey(item, index) {
  if (item.lineId || item.variantData?.clientLineId) return item.lineId || item.variantData.clientLineId;
  return [
...
function isKitchenItemPrepared(order, item, index) {
  ...
  return !!order.preparedItems?.[kitchenOrderItemKey(item, index)];
}
...
async function toggleKitchenItemPrepared(orderId, itemKey) {
  const order = state.orders.find(item => item.id === orderId);
  if (!order || order.status !== "Sedang Disiapkan") return;
  order.preparedItems ||= {};
  order.preparedItems[itemKey] = !order.preparedItems[itemKey];
  const prepared = Boolean(order.preparedItems[itemKey]);
        ...
        ${groups.map(group => {
          ...
          <div class="kitchen-item-group">
            ...
            ${group.items.map((item) => {
              const sourceIndex = (order.items || []).findIndex(candidate => candidate.lineId === item.lineId);
              ...
              return `
                <div class="order-item kitchen-prep-item ${prepared ? "is-ready" : ""}">
                  ${order.status === "Sedang Disiapkan" || order.status === "Selesai" ? `
                    <button class="kitchen-prep-check" type="button" data-kitchen-item-key="${escapeHtml(itemKey)}" onclick='toggleKitchenItemPrepared(${JSON.stringify(order.id)}, ${JSON.stringify(itemKey)})' ${order.status === "Selesai" ? "disabled" : ""} aria-label="Tandai item siap">
                      ${prepared ? "✓" : ""}
                    </button>
                  ` : ""}
  ...
  openModal(`
    <form class="cancel-reason-form" onsubmit="submitCancelOrder(event)">
      ...
      <label class="form-field">
        ...
        <textarea id="cancelReasonInput" rows="4" placeholder="Contoh: pelanggan membatalkan pesanan"></textarea>
      </label>
      <div class="cancel-reason-actions">
        ...
        <button class="btn red" type="submit">Simpan batal</button>
      </div>
    </form>
  `);
...
function renderOrders() {
  ...
  const newOrders = state.orders.filter(order => order.status === "Pesanan Baru");
  const processOrders = state.orders.filter(order => order.status === "Sedang Disiapkan");
  const doneOrders = state.orders.filter(order => order.status === "Selesai");
  const unpaid = state.orders.filter(order => order.paymentStatus !== "Lunas" && order.status !== "Dibatalkan");
  ...
  const statCards = [
    ["Baru", newOrders.length, "Pesanan", "Pesanan Baru"],
    ["Diproses", processOrders.length, "Pesanan", "Sedang Disiapkan"],
    ["Belum Lunas", unpaid.length, "Pesanan", "Belum Lunas"],
    ["Selesai", doneOrders.length, "Pesanan", "Selesai"]
  ];
  return `
    <section class="orders-page">
      <div class="orders-summary">
        ${statCards.map(([label, value, hint, filter]) => `
          <button class="orders-summary-card ${quickFilter === filter ? "primary" : ""}" type="button" onclick="setOrdersQuickFilter('${filter}')">
            <span>${escapeHtml(label)}</span>
...
function orderCenterCard(order, options = {}) {
  ...
  const actions = [
    ...
    order.paymentStatus !== "Lunas" && order.status !== "Dibatalkan" ? `<button class="btn" type="button" onclick="event.preventDefault(); event.stopPropagation(); printUnpaidOrder('${order.id}')">Tagihan</button>` : "",
...
function productCategories() {
  return [...new Set([...(state.categories || []), ...state.products.map(item => item.category).filter(Boolean)])]
  ...
  openConfirmDialog({
    ...
    onConfirm: () => {
      ...
      state.heldOrders = (state.heldOrders || []).map(order => ({
        ...order,
        cart: (order.cart || []).filter(item => item.productId !== id)
      })).filter(order => (order.cart || []).length);
      audit("Produk dihapus", product.name);
      saveState();
      syncMasterEntity("product", { local_id: normalizeMasterLocalId("prod", product.id, product.sku || product.name) }, "delete");
      render();
    }
  });
}
...
function openCategoryManager() {
  ...
  openModal(`
    <div class="section-title category-manager-title">
      <div><h3>Kelola Kategori</h3><p>Edit kategori barang. Kategori kosong bisa dihapus.</p></div>
      <div class="toolbar"><button class="btn" onclick="closeModal()">Tutup</button></div>
    </div>
        ...
        return `
          <div class="addon-manager-row category-manager-row">
            ...
            <div class="row-actions category-manager-actions">
              ...
              <button class="btn red" onclick='deleteCategory(${JSON.stringify(category)})' ${used ? "disabled" : ""}>Hapus</button>
            </div>
          </div>
        `;
...
function renameCategory(event) {
  ...
  if (oldName) {
    ...
    state.categories = productCategories().map(category => category === oldName ? name : category);
    audit("Kategori diperbarui", `${oldName} menjadi ${name}`);
    syncMasterEntity("category", categoryPayload(name));
    syncMasterEntity("category", categoryPayload(oldName), "delete");
    state.products.filter(product => product.category === name).forEach(product => syncMasterEntity("product", product));
  } else {
    state.categories = [...new Set([...(state.categories || []), name])];
...
function deleteCategory(category) {
  if (state.products.some(product => product.category === category)) return toast("Kategori masih dipakai barang.");
  state.categories = (state.categories || []).filter(item => item !== category);
  audit("Kategori dihapus", category);
  saveState();
  syncMasterEntity("category", categoryPayload(category), "delete");
  openCategoryManager();
}
      ...
      ${state.addons.map(addon => `
        <div class="addon-manager-row addon-manager-compact-row">
          ...
          <div class="row-actions addon-manager-compact-actions">
            ...
            <button class="btn ${addon.active ? "" : "accent"}" type="button" onclick="toggleAddon('${addon.id}')">${addon.active ? "Nonaktifkan" : "Aktifkan"}</button>
          </div>
        </div>
...
function saveAddon(event) {
  ...
  const payload = { id, name: form.get("name"), price: Number(form.get("price") || 0), cost: Number(form.get("cost") || 0), active: true };
  if (form.get("id")) state.addons = state.addons.map(addon => addon.id === id ? { ...addon, ...payload } : addon);
  else state.addons.unshift(payload);
  audit(form.get("id") ? "Add-on diperbarui" : "Add-on ditambahkan", payload.name);
  saveState();
  syncMasterEntity("addon", payload);
  openAddonManager();
}
...
function saveInlineAddon(event) {
  event.preventDefault();
  saveInlineAddonFromElement(event.target);
}

function saveInlineAddonFromButton(button) {
  saveInlineAddonFromElement(button.closest("[data-inline-addon-form]"));
}

function saveInlineAddonFromElement(formElement) {
  ...
  const payload = { id, name, price: Number(priceInput?.value || 0), cost: Number(costInput?.value || 0), active: current ? current.active : true };
  if (existingId) state.addons = state.addons.map(addon => addon.id === id ? { ...addon, ...payload } : addon);
  else state.addons.unshift(payload);
  audit(existingId ? "Add-on diperbarui" : "Add-on ditambahkan", payload.name);
  saveState();
  syncMasterEntity("addon", payload);
  refreshProductInlineAddon(formElement.closest("[data-product-addon-manager]"));
}
...
function openProductForm(id) {
  const product = state.products.find(item => item.id === id) || {
    name: "", sku: "", imageName: "", category: "Kopi", description: "", price: 0, cost: 0, stock: 0, minStock: 0,
    active: true, soldOut: false, trackStock: true, prepMinutes: 5, unit: "Porsi", station: "Beverage Bar",
    ...
    variantGroup2: "", variantRequired2: false, variantOptions2: [], allowedAddonIds: []
  };
      ...
      <div class="product-form-body">
          ...
          <div class="product-basic-grid">
            ...
            <div class="grid">
              <div class="grid grid-2 product-price-grid">
                ...
                <div class="field default-price-field ${hasVariant ? "muted-field" : ""}"><label>Harga Jual Tanpa Varian</label><input name="price" class="input price-input" type="number" min="0" required value="${hasVariant ? 0 : product.price}" ${hasVariant ? "readonly" : ""} /><small>${hasVariant ? "Produk memakai harga varian. Harga ini tidak dipakai." : "Dipakai hanya jika mode varian dimatikan."}</small></div>
                <div class="field default-cost-field ${hasVariant ? "muted-field" : ""}"><label>Harga Modal</label><input name="cost" class="input price-input" type="number" min="0" value="${hasVariant ? 0 : product.cost}" ${hasVariant ? "readonly" : ""} /><small>${hasVariant ? "Produk memakai modal varian. Modal ini tidak dipakai." : "Dipakai hanya jika mode varian dimatikan."}</small></div>
              </div>
            </div>
          </div>
        </section>
        <section class="product-tab active" id="product-tab-inventory">
          ...
          <div class="grid grid-2 product-stock-grid">
            <div class="field stock-dependent-field ${product.trackStock ? "" : "muted-field"}"><label>Stok Saat Ini</label><input name="stock" class="input" type="number" min="0" required value="${product.stock}" ${product.trackStock ? "" : "disabled"} /></div>
            <div class="field stock-dependent-field ${product.trackStock ? "" : "muted-field"}"><label>Batas Minimum Stok</label><input name="minStock" class="input" type="number" min="0" required value="${product.minStock}" ${product.trackStock ? "" : "disabled"} /></div>
          </div>
        </section>
        ...
        <section class="product-tab active" id="product-tab-availability">
          ...
          <div class="product-status-actions">
            <label class="btn product-active-toggle"><input name="active" type="checkbox" ${product.active ? "checked" : ""} /> Aktif dijual</label>
            <label class="btn product-active-toggle"><input name="soldOut" type="checkbox" ${product.soldOut ? "checked" : ""} /> Tandai sedang habis</label>
          </div>
          <div class="product-form-footer">
            <button class="btn accent" type="submit">Simpan Barang</button>
          </div>
        </section>
      </div>
    </form>
...
function saveProduct(event, id) {
  ...
  if (category) state.categories = [...new Set([...(state.categories || []), category])];
  if (id) state.products = state.products.map(product => product.id === id ? payload : product);
  else state.products.unshift(payload);
  audit(id ? "Produk diperbarui" : "Produk ditambahkan", payload.name);
  saveState();
  syncMasterEntity("product", payload);
  closeModal();
  render();
}
...
function renderStockV2() {
  const products = [...state.products].sort((a, b) => Number(a.soldOut) - Number(b.soldOut) || a.stock - b.stock);
  return `
    <div class="grid grid-2">
      <div class="card">
        <div class="section-title"><div><h3>Status Stok</h3><p>Produk tanpa stok tetap bisa ditandai habis atau tersedia.</p></div></div>
        ${products.map(product => {
          const stateText = product.soldOut ? "Habis" : !product.trackStock ? "Tanpa stok" : product.stock <= 0 ? "Habis" : product.stock <= product.minStock ? "Menipis" : "Aman";
          return `<div class="list-row"><div class="line"><strong>${escapeHtml(product.name)}</strong><span class="pill ${stateText === "Habis" ? "cancel" : stateText === "Menipis" ? "process" : "ready"}">${stateText}</span></div><div class="line muted"><span>${product.trackStock ? `Stok ${product.stock} &middot; Minimum ${product.minStock}` : "Tidak mengurangi stok saat dijual"}</span><div class="row-actions">${product.trackStock ? `<button class="btn" onclick="openStockFormV2('${product.id}')">Atur</button>` : ""}<button class="btn" onclick="toggleProductSoldOut('${product.id}')">${product.soldOut ? "Tersedia" : "Tandai Habis"}</button></div></div></div>`;
        }).join("")}
      </div>
      <div class="card">
        <div class="section-title"><div><h3>Riwayat Stok</h3><p>Pergerakan otomatis dari transaksi dan pembatalan.</p></div></div>
...
function reportSourceRecords(range = null) {
  ...
  ].filter(record => isReportableRecord(record) && recordInRange(record, range));
  const merged = new Map();
...
function saveSelfOrderSettings(event) {
  ...
  saveState();
  toast("Setting self order disimpan.");
  render();
}

...
function updatePosSearch(input) {
  ...
  refreshPosFast({ products: true, preserveScroll: true, focusSearch: true, cursor });
}
...
function setCategory(category) {
  ...
  refreshPosFast({ categories: true, products: true, preserveScroll: false });
}
...
function rememberSelfOrderMenuScroll() {
...
function posVisibleProducts() {
  ...
  return stableProducts(state.products)
    .filter(product => product.active && product.channelPOS !== false)
...
function renderPosCategoryCards() {
  ...
  return `
    <div class="category-cards">
      ${posCategories().map(cat => `<button class="category-card ${cat === selected ? "active" : ""}" type="button" onclick="setCategory(${JSON.stringify(cat).replaceAll('"', "&quot;")})"><strong>${escapeHtml(cat)}</strong><span>${categoryCount(cat)} item</span></button>`).join("")}
    </div>
  `;
}
...
function renderPosProductList() {
  ...
  const products = posVisibleProducts();
  return `
    <div class="product-list">
      ...
      ${products.map(product => productCard(product)).join("") || empty("Produk tidak ditemukan.")}
    </div>
  `;
}
...
function productCard(product) {
  ...
  return `
    <button class="product-card" data-pos-product-card data-product-id="${escapeHtml(product.id)}" onclick="addToCart('${product.id}')" ${disabled ? "disabled" : ""}>

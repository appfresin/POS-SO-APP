...
async function deleteSupabaseMaster(entity, payload) {
  ...
  if (localId) {
    if (entity === "product") {
      ...
      if (productRow?.id) {
        const { error: relationError } = await supabaseClient.from("product_addons").delete().eq("product_id", productRow.id);
...
async function selfOrderCreateOrder() {
  ...
  if (!synced) {
    ...
    saveState();
    render();
    return toast("Pesanan belum terkirim ke dapur. Cek koneksi lalu coba lagi.");
  }
  selfOrderCart = [];
  saveSelfOrderCart();
  ...
  openConfirmDialog({
    ...
    onConfirm: () => {
      ...
      deletedProducts.forEach(item => {
        syncMasterEntity("product", { local_id: normalizeMasterLocalId("prod", item.id, item.sku || item.name) }, "delete");
      });
      render();
    }
  });
...
function saveAddon(event) {
  ...
  const id = form.get("id") || existingByName?.id || uid();
  const current = state.addons.find(addon => addon.id === id);
  const payload = { id, name, price: Number(form.get("price") || 0), cost: Number(form.get("cost") || 0), active: current ? current.active : true };
  if (form.get("id") || existingByName) state.addons = state.addons.map(addon => addon.id === id ? { ...addon, ...payload } : addon);
  else state.addons.unshift(payload);
  state.addons = uniqueAddons(state.addons);
  audit(form.get("id") || existingByName ? "Add-on diperbarui" : "Add-on ditambahkan", payload.name);
  saveState();
...
function deleteAddon(id) {
  ...
  saveState();
  syncMasterEntity("addon", addonPayload(addon), "delete");
  affectedProducts.forEach(product => syncMasterEntity("product", product));
  openAddonManager();
}
...
function saveInlineAddonFromElement(formElement) {
  ...
  if (!name) return toast("Nama add-on belum diisi.");
  const existingByName = state.addons.find(addon => addon.name.trim().toLowerCase() === name.toLowerCase());
  const id = existingId || existingByName?.id || uid();
  const current = state.addons.find(addon => addon.id === id);
  const payload = { id, name, price: Number(priceInput?.value || 0), cost: Number(costInput?.value || 0), active: current ? current.active : true };
  if (existingId || existingByName) state.addons = state.addons.map(addon => addon.id === id ? { ...addon, ...payload } : addon);
  else state.addons.unshift(payload);
  state.addons = uniqueAddons(state.addons);
  audit(existingId || existingByName ? "Add-on diperbarui" : "Add-on ditambahkan", payload.name);
  saveState();
  syncMasterEntity("addon", payload);
...
async function saveProduct(event, id) {
  ...
  try {
    ...
    saveState();
    await syncMasterEntity("product", payload);
    closeModal();
    render();
  } catch (error) {

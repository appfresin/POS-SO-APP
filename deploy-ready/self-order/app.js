...
function normalizeState() {
  ...
  state.orders ||= [];
  state.syncQueue ||= [];
  state.syncQueue = state.syncQueue.filter(item => !(item.type === "master" && item.operation !== "delete" && item.userInitiated !== true));
  state.importedSales ||= [];
  ...
  state.deletedProductKeys ||= [];
  state.deletedProductKeys = [...new Set(state.deletedProductKeys.map(key => String(key || "").trim()).filter(Boolean))].slice(-1000);
  state.deletedAddonKeys ||= [];
  state.deletedAddonKeys = [...new Set(state.deletedAddonKeys.map(key => String(key || "").trim()).filter(Boolean))].slice(-1000);
  state.deletedCategoryKeys ||= [];
  state.deletedCategoryKeys = [...new Set(state.deletedCategoryKeys.map(key => String(key || "").trim()).filter(Boolean))].slice(-1000);
...
function productUniqueKey(product) {
  ...
  if (name) return ["name", name, price].join("|");
...
function productDeleteKeys(product) {
  ...
  localIds.forEach(value => keys.push(`local:${value}`));
  remoteIds.forEach(value => keys.push(`remote:${value}`));
  return [...new Set(keys)];
}

function rememberDeletedProduct(product) {
  const keys = productDeleteKeys(product);
  if (!keys.length) return;
  state.deletedProductKeys ||= [];
  state.deletedProductKeys = [...new Set([...state.deletedProductKeys, ...keys])].slice(-1000);
}

function isDeletedProduct(product) {
  const deletedKeys = new Set(state.deletedProductKeys || []);
  return productDeleteKeys(product).some(key => deletedKeys.has(key));
}

function addonDeleteKeys(addon) {
  const keys = [];
  [addon?.local_id, addon?.localId, addon?.id].map(value => String(value || "").trim()).filter(Boolean).forEach(value => keys.push(`local:${value}`));
  [addon?.supabaseId, addon?.supabase_id, addon?.remoteId].map(value => String(value || "").trim()).filter(Boolean).forEach(value => keys.push(`remote:${value}`));
  const name = String(addon?.name || "").trim().toLowerCase();
  if (name) keys.push(`name:${name}`);
  return [...new Set(keys)];
}

function categoryDeleteKeys(category) {
  const row = typeof category === "string" ? { name: category } : category || {};
  const keys = [];
  [row.local_id, row.localId, row.id].map(value => String(value || "").trim()).filter(Boolean).forEach(value => keys.push(`local:${value}`));
  [row.supabaseId, row.supabase_id, row.remoteId].map(value => String(value || "").trim()).filter(Boolean).forEach(value => keys.push(`remote:${value}`));
...
function rememberDeletedAddon(addon) {
  const keys = addonDeleteKeys(addon);
  if (!keys.length) return;
  state.deletedAddonKeys ||= [];
  state.deletedAddonKeys = [...new Set([...state.deletedAddonKeys, ...keys])].slice(-1000);
}

function rememberDeletedCategory(category) {
  const keys = categoryDeleteKeys(category);
  if (!keys.length) return;
  state.deletedCategoryKeys ||= [];
  state.deletedCategoryKeys = [...new Set([...state.deletedCategoryKeys, ...keys])].slice(-1000);
}

function forgetDeletedAddon(addon) {
  const keys = new Set(addonDeleteKeys(addon));
  state.deletedAddonKeys = (state.deletedAddonKeys || []).filter(key => !keys.has(key));
}

function forgetDeletedCategory(category) {
  const keys = new Set(categoryDeleteKeys(category));
  state.deletedCategoryKeys = (state.deletedCategoryKeys || []).filter(key => !keys.has(key));
}

function isDeletedAddon(addon) {
  const deletedKeys = new Set(state.deletedAddonKeys || []);
  return addonDeleteKeys(addon).some(key => deletedKeys.has(key));
}

function isDeletedCategory(category) {
  const deletedKeys = new Set(state.deletedCategoryKeys || []);
  return categoryDeleteKeys(category).some(key => deletedKeys.has(key));
}
...
function normalizeStaffState() {
  ...
  if (!activeStillValid) state.staff.activeStaffId = state.staff.members[0]?.id || "";
}
...
function markMasterSyncPending(entity, payload, operation = "upsert", error = null, options = {}) {
  state.syncQueue ||= [];
  ...
  const queueItem = {
    ...
    operation,
    payload,
    userInitiated: options.userInitiated === true || existing?.userInitiated === true,
    attempts,
    lastError: error?.message || "Koneksi Supabase belum tersedia",
    lastAttemptAt: new Date(now).toISOString(),
    nextAttemptAt: new Date(now + syncRetryDelayMs(attempts)).toISOString()
...
async function writeProductPayload(productId, payload) {
  let { error } = await supabaseClient.from("products").update(payload).eq("id", productId);
  if (isMissingStockOpnameColumnError(error)) {
    supabaseStockOpnameColumnsReady = false;
    ({ error } = await supabaseClient.from("products").update(productPayloadWithoutStockOpname(payload)).eq("id", productId));
  }
  if (error) throw error;
}

async function insertProductPayload(payload) {
  let { data, error } = await supabaseClient.from("products").insert(payload).select("id").single();
  if (isMissingStockOpnameColumnError(error)) {
    supabaseStockOpnameColumnsReady = false;
    ({ data, error } = await supabaseClient.from("products").insert(productPayloadWithoutStockOpname(payload)).select("id").single());
  }
  return { data, error };
}
...
async function deleteSupabaseMaster(entity, payload) {
  ...
  if (entity === "product") {
    ...
    for (const productId of remoteIds) {
      const { error: relationError } = await supabaseClient.from("product_addons").delete().eq("product_id", productId);
      if (relationError && !isSupabaseForeignKeyError(relationError)) throw relationError;
  ...
  if (entity === "addon") {
    ...
    for (const addonId of remoteIds) {
      const { error: relationError } = await supabaseClient.from("product_addons").delete().eq("addon_id", addonId);
      if (relationError && !isSupabaseForeignKeyError(relationError)) throw relationError;
      const { error } = await supabaseClient.from("addons").delete().eq("id", addonId);
      if (error && !isSupabaseForeignKeyError(error)) throw error;
    }
    if (localId) {
      const { error } = await supabaseClient.from("addons").delete().eq("local_id", localId);
      if (error && !isSupabaseForeignKeyError(error)) throw error;
    }
    return;
  }
  if (entity === "category") {
    ...
    if (localId) {
      const { error } = await supabaseClient.from("categories").delete().eq("local_id", localId);
      if (error && !isSupabaseForeignKeyError(error)) throw error;
      return;
    }
    if (payload.name) {
      const { error } = await supabaseClient.from("categories").delete().eq("name", payload.name);
      if (error && !isSupabaseForeignKeyError(error)) throw error;
    }
    return;
  }
...
async function syncMasterEntity(entity, payload, operation = "upsert", options = {}) {
  ...
  if (!canAttemptSupabaseSync()) {
    markMasterSyncPending(entity, payload, operation, null, options);
    saveState();
    return false;
  }
  try {
    await syncMasterQueueItem({
      type: "master",
      entity,
      payload,
      operation,
    ...
    markMasterSyncPending(entity, payload, operation, error, options);
    saveState();
    return false;
  }
}
    ...
    for (const item of pendingItems) {
      if (item.type === "master") {
        try {
          ...
          markMasterSyncPending(item.entity, item.payload, item.operation, error, { userInitiated: item.userInitiated === true });
          saveState();
        }
        continue;
      }
...
function clearLocalMasterDataAfterEmptyRemote() {
  state.products = [];
  state.addons = [];
  state.categories = [];
  state.syncQueue = (state.syncQueue || []).filter(item => !(item.type === "master" && item.operation !== "delete"));
  normalizeState();
  saveState();
}

function userInitiatedMasterUpserts(entity) {
  return (state.syncQueue || [])
    .filter(item => item.type === "master" && item.operation !== "delete" && item.entity === entity && item.userInitiated === true)
    .map(item => item.payload)
    .filter(Boolean);
}

function hasUserInitiatedMasterUpserts() {
  return (state.syncQueue || []).some(item => item.type === "master" && item.operation !== "delete" && item.userInitiated === true);
}

function productFromQueuedPayload(payload) {
  return {
    id: payload.id || payload.localId || payload.local_id || uid(),
    name: payload.name || "",
    sku: payload.sku || "",
    ...
    minStock: Number(payload.minStock || payload.min_stock || 0),
    unit: payload.unit || "item",
    trackStock: payload.trackStock ?? payload.track_stock ?? true,
    active: payload.active !== false,
    soldOut: Boolean(payload.soldOut || payload.sold_out),
    prepMinutes: Number(payload.prepMinutes || payload.prep_minutes || 5),
    station: payload.station || "Dapur",
    imageName: payload.imageName || payload.image_name || "",
    kitchenNote: payload.kitchenNote || payload.kitchen_note || "",
    channelPOS: payload.channelPOS ?? payload.channel_pos ?? true,
    channelSelfOrder: Boolean(payload.channelSelfOrder ?? payload.channel_self_order),
    channelDelivery: payload.channelDelivery ?? payload.channel_delivery ?? true,
    variantMode: payload.variantMode || payload.variant_mode || "off",
    variantGroup: payload.variantGroup || payload.variant_group || "",
    variantRequired: Boolean(payload.variantRequired || payload.variant_required),
    variantOptions: payload.variantOptions || payload.variant_options || [],
    allowedAddonIds: payload.allowedAddonIds || payload.allowed_addon_ids || [],
    soldOutVariants: payload.soldOutVariants || payload.sold_out_variants || {},
    syncStatus: "pending"
  };
}

function pendingUserInitiatedMasterState() {
  const products = userInitiatedMasterUpserts("product").map(productFromQueuedPayload).filter(product => product.name);
  const addons = userInitiatedMasterUpserts("addon").map(addon => ({
    id: addon.id || addon.localId || addon.local_id || uid(),
    name: addon.name || "",
    price: Number(addon.price || 0),
    cost: Number(addon.cost || 0),
    active: addon.active !== false,
    soldOut: Boolean(addon.soldOut || addon.sold_out)
  })).filter(addon => addon.name);
  const categories = [
    ...userInitiatedMasterUpserts("category").map(category => category.name).filter(Boolean),
...
function clearLocalMasterDataForRemoteAuthority(options = {}) {
  ...
  const pending = pendingUserInitiatedMasterState();
  state.products = pending.products;
  state.addons = pending.addons;
  state.categories = [...new Set(pending.categories)];
  normalizeState();
...
async function loadStoreSettingsFromSupabase(options = {}) {
  ...
  try {
    ...
    if (settings) {
      const { staffSync, ...storeSettings } = settings;
...
async function loadMasterDataFromSupabase(options = {}) {
  ...
  try {
    ...
    const allServerCategories = (categoryResult.data || [])
      .map(row => ({ id: row.id, local_id: row.local_id, name: normalizeSupabaseCategory(row) }))
      .filter(category => category.name);
    const serverCategories = allServerCategories.filter(category => !isDeletedCategory(category)).map(category => category.name);
    const deletedServerCategories = allServerCategories.filter(category => isDeletedCategory(category));
    const allServerAddons = (addonResult.data || []).map(normalizeSupabaseAddon).filter(addon => addon.name);
    const serverAddons = allServerAddons.filter(addon => !isDeletedAddon(addon));
    const deletedServerAddons = allServerAddons.filter(addon => isDeletedAddon(addon));
    ...
    const localProductById = new Map(state.products.map(product => [product.id, product]));
    const localProductBySupabaseId = new Map(state.products.filter(product => product.supabaseId).map(product => [product.supabaseId, product]));
    const allServerProducts = (productResult.data || [])
      .map(row => {
        const product = mergeProductLocalMeta(
          normalizeSupabaseProduct(row, row.categories?.name || ""),
          localProductById.get(row.local_id || row.id) || localProductBySupabaseId.get(row.id)
        );
    ...
    const serverProducts = allServerProducts.filter(product => !isDeletedProduct(product));
    const deletedServerProducts = allServerProducts.filter(product => isDeletedProduct(product));

    const remoteMasterIsEmpty = !allServerProducts.length && !allServerAddons.length && !allServerCategories.length;
    if (remoteMasterIsEmpty && (state.products.length || state.addons.length || state.categories.length)) {
      if (hasUserInitiatedMasterUpserts()) {
        clearLocalMasterDataForRemoteAuthority();
      ...
      clearLocalMasterDataAfterEmptyRemote();
      masterSyncLoaded = true;
      masterSyncLoadedAt = Date.now();
      if (shouldRenderAfterRealtimeLoad("master")) render();
      return;
    ...
    const pending = pendingUserInitiatedMasterState();
    const serverProductIds = new Set(serverProducts.flatMap(product => productDeleteKeys(product)));
    const pendingProducts = pending.products.filter(product => !productDeleteKeys(product).some(key => serverProductIds.has(key)));
    const serverAddonIds = new Set(serverAddons.map(addon => addon.id));
    const pendingAddons = pending.addons.filter(addon => !serverAddonIds.has(addon.id));
    state.categories = [...new Set(serverCategories)];
    state.addons = uniqueAddons([...serverAddons, ...pendingAddons]);
    state.products = [...serverProducts, ...pendingProducts];
    normalizeState();
    saveState();

    for (const category of deletedServerCategories) syncMasterEntity("category", category, "delete");
    for (const addon of deletedServerAddons) syncMasterEntity("addon", addon, "delete");
    for (const product of deletedServerProducts) syncMasterEntity("product", product, "delete");
...
async function removeStaffMember(id) {
  ...
  if (state.staff.activeStaffId === id) state.staff.activeStaffId = state.staff.members[0]?.id || "";
  normalizeStaffState();
  saveState();
...
function toggleProductSoldOut(id) {
  ...
  saveState();
  syncMasterEntity("product", product).finally(() => productActionLocks.delete(id));
  render();
}
...
function deleteProduct(id) {
  ...
  openConfirmDialog({
    ...
    danger: true,
    onConfirm: () => {
      const deleteKey = productUniqueKey(product);
      const deletedProducts = state.products.filter(item => item.id === id || productUniqueKey(item) === deleteKey);
      deletedProducts.forEach(rememberDeletedProduct);
      const deletedIds = new Set(deletedProducts.map(item => item.id));
      state.products = state.products.filter(item => !deletedIds.has(item.id));
      ...
      deletedProducts.forEach(item => {
        syncMasterEntity("product", {
          ...item,
          local_id: normalizeMasterLocalId("prod", item.id, item.sku || item.name)
        }, "delete");
      });
      render();
    }
...
function renameCategory(event) {
  ...
  if (oldName) {
    ...
    rememberDeletedCategory(oldName);
    ...
    syncMasterEntity("category", categoryPayload(name), "upsert", { userInitiated: true });
    syncMasterEntity("category", categoryPayload(oldName), "delete");
    state.products.filter(product => product.category === name).forEach(product => syncMasterEntity("product", product, "upsert", { userInitiated: true }));
  } else {
    forgetDeletedCategory(name);
...
function deleteCategory(category) {
  ...
  openConfirmDialog({
    ...
    detail: "Kategori kosong akan dihapus dari lokal dan Supabase.",
    confirmLabel: "Hapus",
    danger: true,
    onConfirm: () => {
...
function saveAddonPayload(input) {
  ...
  if (input.id || existingByName) state.addons = state.addons.map(addon => addon.id === id ? { ...addon, ...payload } : addon);
  else state.addons.unshift(payload);
  state.addons = uniqueAddons(state.addons);
  audit(input.id || existingByName ? "Add-on diperbarui" : "Add-on ditambahkan", payload.name);
  saveState();
  syncMasterEntity("addon", payload, "upsert", { userInitiated: true });
  return payload;
}
  ...
  openConfirmDialog({
    ...
    onConfirm: () => {
      ...
      state.products = state.products.map(product => {
        if (!Array.isArray(product.allowedAddonIds) || !product.allowedAddonIds.includes(id)) return product;
        const next = { ...product, allowedAddonIds: product.allowedAddonIds.filter(addonId => addonId !== id) };
        affectedProducts.push(next);
        return next;
      });
      state.addons = state.addons.filter(item => item.id !== id);
      audit("Add-on dihapus", addon.name);
      saveState();
      syncMasterEntity("addon", addon, "delete");
      affectedProducts.forEach(product => syncMasterEntity("product", product, "upsert", { userInitiated: true }));
...
async function saveProduct(event, id) {
  ...
  if (submitButton) {
    ...
    submitButton.innerHTML = `<span class="btn-spinner" aria-hidden="true"></span><span>Menyimpan...</span>`;
  }
  ...
  const payload = {
    ...
    trackStock,
    stockOpname: form.get("stockOpname") === "on",
    stockOpnameCheckedAt: form.get("stockOpname") === "on" ? existingProduct.stockOpnameCheckedAt || "" : "",
    stockOpnameCheckedBy: form.get("stockOpname") === "on" ? existingProduct.stockOpnameCheckedBy || "" : ""
  };
  try {
    if (category) {
      forgetDeletedCategory(category);
    ...
    saveState();
    await syncMasterEntity("product", payload, "upsert", { userInitiated: true });
    closeModal();
    render();
  } catch (error) {
    console.error("Save product failed", error);
    toast("Gagal menyimpan barang. Coba lagi.");
    if (submitButton) {
      submitButton.disabled = false;

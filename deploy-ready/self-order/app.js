...
function forgetDeletedProduct(product) {
  state.deletedProducts = normalizeDeletedProductMarkers(state.deletedProducts)
    .filter(marker => !productMatchesDeletedMarker(product, marker));
}
...
async function syncMasterQueueItem(item) {
  ...
  if (item.entity === "product" && isDeletedProduct(item.payload)) return;
...
async function saveProduct(event, id) {
  ...
  if (wantsStockOpname && !supabaseStockOpnameColumnsReady) {
    ...
    resetProductSaveButton(submitButton);
    return;
  }
  ...
  try {
    ...
    const synced = await syncMasterEntity("product", payload, "upsert", { userInitiated: true });
    if (!synced) throw new Error("Produk belum tersimpan ke Supabase.");

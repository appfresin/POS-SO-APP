...
document.addEventListener("focusout", () => {
  ...
  window.setTimeout(() => {
    if (isUserEditingInteractiveElement()) return;
...
function productUniqueKey(product) {
  return [
    ...
    String(productDisplayPrice(product) || "").trim().toLowerCase()
  ].join("|");
}
...
function shouldRenderAfterRealtimeLoad(kind) {
  if (isUserEditingInteractiveElement()) {
...
async function reloadStateFromLocalPersistence() {
  try {
    ...
    normalizeState();
    if (!isUserEditingInteractiveElement() && (view !== "selforder" || selfOrderStep() === "menu")) render();
...
async function loadProfitSummary(level = "years", year = null, month = null, force = false) {
  ...
  if (profitSummaryUnavailable && !force) return;
  ...
  try {
    ...
    const missingReportView = error?.code === "PGRST205" || String(error?.message || "").toLowerCase().includes("report_profit");
...
function requestActiveReportData(force = false) {
  ...
  if (section === "profit") {
    if (!profitSummaryUnavailable || force) loadProfitSummary("years", null, null, force);
...
function categoryCount(category) {
  const products = stableProducts(state.products).filter(product => product.active);
...
function renderSelfOrderCatalog() {
  ...
  const visibleCategories = categories.filter(item => item !== "Semua" && activeProducts.some(product => product.category === item));
  return `

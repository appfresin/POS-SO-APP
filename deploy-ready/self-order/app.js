...
function scheduleNativePushOrderRefresh(route = "orders", attempt = 0, target = {}) {
  ...
  try {
    const shouldNavigate = normalizedTarget.navigate || ORDER_NOTIFICATION_VIEWS.has(view);
...
function selfOrderUpsellProducts(context = "cart", limit = 2) {
  ...
  const products = ids
    .map(id => selfOrderProduct(id))
    .filter(Boolean)
    .filter(product => !selfOrderCartHasProduct(product.id));
  ...
  const offset = selfOrderHashText(seed) % products.length;
  return [...products.slice(offset), ...products.slice(0, offset)].slice(0, limit);
}
      ...
      <div class="self-order-upsell-list">
        ${products.map(product => `
          <article class="self-order-upsell-item">
            <span class="self-order-upsell-media">${productImageOrToken(product, "self-order-upsell-img", 240)}</span>
            <strong>${escapeHtml(product.name)}</strong>
            <span class="self-order-upsell-price">+ ${productDisplayPrice(product)}</span>
            <button type="button" onclick="selfOrderAddUpsellProduct('${escapeHtml(product.id)}')" aria-label="Tambah ${escapeHtml(product.name)}">+</button>
          </article>
        `).join("")}
      </div>
    </section>
...
async function selfOrderAddUpsellProduct(productId) {
  const product = selfOrderProduct(productId);
  if (!product) return toast("Produk rekomendasi tidak tersedia.");
  if (selfOrderVariantGroups(product).length) {
    selfOrderOpenProduct(product.id);
    return;
  }
  const variantKey = "";
  const limit = productSaleStockLimit(product, variantKey);
  const existingLine = selfOrderCart.find(item => item.productId === product.id && !(item.variantSelections || []).length && !(item.addons || []).length);
  ...
  if (existingLine) {
    ...
    selfOrderCart = [...selfOrderCart, { ...buildCartItem(product), addons: [], note: "", qty: 1 }];
  }
  saveSelfOrderCart();
  render();
  if (limitedStockRequiresOnline(product, variantKey) && canAttemptSupabaseSync()) {
...
async function selfOrderChangeQty(lineId, delta) {
  ...
  if (delta > 0 && limitedStockRequiresOnline(product, variantKey) && !canAttemptSupabaseSync()) return toast(limitedStockServiceMessage(navigator.onLine === false ? "offline" : "service_unavailable"));
  ...
  return `
    <section class="orders-page">
      ...
      <div class="orders-list-grid" data-orders-list>
        ${ordered.map(order => orderCenterCard(order, { searchQuery: query, sequenceNumber: visibleSequences.get(order.id) || 0 })).join("")}

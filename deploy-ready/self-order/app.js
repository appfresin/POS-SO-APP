...
function applyNotificationSettings(settings) {
  const { deviceNotificationsEnabled, ...storeSettings } = settings || {};
...
async function registerNativePushToken(options = {}) {
  ...
  const registerKey = `${session.user.id}|${scopeKey}|${target}|${enabled ? "on" : "off"}|${token}`;
...
function notifyIncomingOrders(orders = [], previousKeys = orderNotificationKnownKeys, reason = "") {
  ...
  if (isKasirinAndroidWebView()) return newOrders;
...
function scheduleNativePushOrderRefresh(route = "orders", attempt = 0) {
  const normalizedRoute = ["kitchen", "orders"].includes(route) ? route : "orders";
  const nextAttempt = Number(attempt || 0);
  try {
    if (typeof go === "function" && view !== normalizedRoute) go(normalizedRoute);
    if (supabaseReadable() && localDbReady && !realtimeOrdersLoading) {
      Promise.resolve(loadRecentOrdersFromSupabase({
        ...
        if (!state.orders.some(order => isNewOrderStatus(order) && !order.cancelledAt) && nextAttempt < 10) {
          setTimeout(() => scheduleNativePushOrderRefresh(normalizedRoute, nextAttempt + 1), nextAttempt < 3 ? 900 : 2200);
        }
      }).catch(() => {
        if (nextAttempt < 10) setTimeout(() => scheduleNativePushOrderRefresh(normalizedRoute, nextAttempt + 1), 2200);
      });
      return;
    }
  } catch (error) {
  ...
  if (nextAttempt < 12) {
    setTimeout(() => scheduleNativePushOrderRefresh(normalizedRoute, nextAttempt + 1), nextAttempt < 4 ? 800 : 1800);
  }
}
...
function topActions() {
  ...
  if (view === "stock-opname") {
    return `
      <button class="stock-opname-qr-btn" type="button" onclick="openStockOpnameQrModal()" title="QR Stok Opname" aria-label="Tampilkan QR Stok Opname">
        ${navIcon("qr")}
      </button>
    `;
  }
...
function receiptPrintModel(order = null) {
  ...
  return {
    ...
    totalLines: [
      ...
      ...(hasReceivedAmount ? [{ label: "Bayar", value: receiptMoneyText(receivedAmount) }] : []),
...
async function reserveLimitedStockQty(product, variantKey = "", desiredQty = 0, source = limitedStockSource()) {
  ...
  if (!canAttemptSupabaseSync()) return { accepted: false, available: 0, reserved: 0, reason: "offline" };
  const key = limitedStockReservationKey(product.id, variantKey);
...
function selfOrderProductCanDisplay(product) {
  ...
  return variants.some(variant => !isProductVariantSoldOut(product, variant.key));
}
...
async function selfOrderShowPayment() {
  ...
  if (!stockReady.ok) {
    return toast(stockReady.reason === "offline" || stockReady.reason === "unavailable"
      ? "Produk dengan stok terbatas memerlukan koneksi internet"
      : limitedStockMessage(stockReady.product, stockReady.variantKey, stockReady.available));
  }
  syncSelfOrderTableFromUrl();
...
function selfOrderDetailCartState(product) {
  ...
  return { selections, missingRequired, addons, line };
}
...
async function selfOrderApplyDetailQty(product, detailState, nextQty) {
  ...
  if (increasing && limitedStockRequiresOnline(product, variantKey) && !canAttemptSupabaseSync()) {
    return toast("Produk dengan stok terbatas memerlukan koneksi internet");
  }
  ...
  if (detailState.line) {
    if (nextQty <= 0) {
      ...
      selfOrderCart = selfOrderCart.map(item => item.lineId === detailState.line.lineId ? { ...item, qty: nextQty } : item);
      saveSelfOrderCart();
  ...
  if (limitedStockRequiresOnline(product, variantKey) && canAttemptSupabaseSync()) {
    ...
    if (!result.accepted) {
      ...
      return toast(result.reason === "offline" || result.reason === "unavailable"
        ? "Produk dengan stok terbatas memerlukan koneksi internet"
        : limitedStockMessage(product, variantKey, result.available));
    }
  }
  if (nextQty <= 0) {
...
async function selfOrderUpsertDetailCartItem(product, detailState) {
  ...
  if (limitedStockRequiresOnline(product, variantKey) && !canAttemptSupabaseSync()) {
    toast("Produk dengan stok terbatas memerlukan koneksi internet");
    return false;
  }
  const previousCart = selfOrderCart.map(item => ({ ...item }));
  if (detailState.line) {
    selfOrderCart = selfOrderCart.map(item => item.lineId === detailState.line.lineId ? { ...item, qty } : item);
    saveSelfOrderCart();
  } else {
    const cartItem = { ...buildCartItem(product, detailState.selections), addons: detailState.addons, note: "", qty };
  ...
  if (limitedStockRequiresOnline(product, variantKey)) {
    ...
    if (!result.accepted) {
      selfOrderCart = previousCart;
      saveSelfOrderCart();
      toast(result.reason === "offline" || result.reason === "unavailable"
        ? "Produk dengan stok terbatas memerlukan koneksi internet"
        : limitedStockMessage(product, variantKey, result.available));
      return false;
    }
...
async function selfOrderChangeQty(lineId, delta) {
  ...
  const nextQty = Math.max(0, Number(line?.qty || 0) + Number(delta || 0));
  const variantKey = cartItemVariantKey(line);
  const limit = productSaleStockLimit(product, variantKey);
  if (product && Number.isFinite(limit) && limitedStockCartQty(selfOrderCart, product.id, variantKey, lineId) + nextQty > limit) {
    return toast(limitedStockMessage(product, variantKey, limit));
  }
  if (delta > 0 && limitedStockRequiresOnline(product, variantKey) && !canAttemptSupabaseSync()) return toast("Produk dengan stok terbatas memerlukan koneksi internet");
  if (limitedStockPendingKeys.has(limitedStockReservationKey(product?.id, variantKey))) return;
  const previousCart = selfOrderCart.map(item => ({ ...item }));
...
async function selfOrderCreateOrder() {
  ...
  if (!stockCommit.accepted) {
    ...
    selfOrderSubmitError = stockCommit.reason === "unavailable"
      ? "Produk dengan stok terbatas memerlukan koneksi internet"
      : limitedStockMessage(failedProduct, stockCommit.variantKey || "", stockCommit.available || 0);
    return;
  }
      ...
      <section class="self-order-detail">
        ...
        <form class="self-order-detail-panel" onsubmit="event.preventDefault(); selfOrderAddProduct('${product.id}')">
          ...
          <div class="self-order-detail-actions">
            ...
            <button class="self-order-primary" data-self-order-add-btn type="submit" ${readyToAdd ? "" : "disabled"}>Tambah ke Keranjang</button>
          </div>
        </form>
      </section>
    </main>
...
function availableSelfOrderVariantOptions(product, group) {
  return (group?.options || []).filter(option => !isProductVariantSoldOut(product, selfOrderVariantKey(product, option)));
}
...
function selfOrderUpdateAddButton(productId) {
  ...
  button.disabled = !selfOrderDetailReadyToAdd(product);
}
...
function productCard(product) {
  ...
  const disabled = saleLocked || product.soldOut || (variants.length
    ? variants.every(variant => isProductVariantSoldOut(product, variant.key))
...
async function addToCart(id) {
  ...
  if (limitedStockRequiresOnline(product) && !canAttemptSupabaseSync()) return toast("Produk dengan stok terbatas memerlukan koneksi internet");
  ...
  if (limitedStockRequiresOnline(product)) {
    ...
    if (!result.accepted) {
      ...
      toast(result.reason === "offline" || result.reason === "unavailable"
        ? "Produk dengan stok terbatas memerlukan koneksi internet"
        : limitedStockMessage(product, "", result.available));
    }
  }
}
...
async function setCartQty(id, qty) {
  ...
  if (increasing && limitedStockRequiresOnline(product, variantKey) && !canAttemptSupabaseSync()) return toast("Produk dengan stok terbatas memerlukan koneksi internet");
  if (limitedStockPendingKeys.has(limitedStockReservationKey(product?.id, variantKey))) return;
  const previousCart = cart.map(item => ({ ...item }));
  if (qty <= 0) cart = cart.filter(item => cartItemKey(item) !== id);
  else {
    if (product && limitedStockCartQty(cart, product.id, variantKey, id) + qty > productSaleStockLimit(product, variantKey)) {
...
async function resumeHeldOrder(id) {
  ...
  if (!stockReady.ok) {
    toast(stockReady.reason === "offline" || stockReady.reason === "unavailable"
      ? "Produk dengan stok terbatas memerlukan koneksi internet"
      : limitedStockMessage(stockReady.product, stockReady.variantKey, stockReady.available));
  }
}
...
async function submitOrder() {
  ...
  if (!stockCommit.accepted) {
    if (stockCommit.reason === "offline" || stockCommit.reason === "unavailable") return toast("Produk dengan stok terbatas memerlukan koneksi internet");
    return toast(limitedStockMessage(stockCommit.product || stockValidation.product, stockCommit.variantKey || "", stockCommit.available || 0));
  }
  order.limitedStockCommitToken = stockCommit.token || "";

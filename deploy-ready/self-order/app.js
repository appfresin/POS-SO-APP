        ...
        <div class="cart-payment-page">
          <div class="payment-detail-shell">
            ...
            <div class="payment-detail-fields">
              ...
              <div class="field ${paymentMethodInvalid ? "invalid" : ""}" data-mobile-label="Bayar"><label>Pembayaran</label><select id="paymentMethod" aria-invalid="${paymentMethodInvalid ? "true" : "false"}" aria-describedby="posPaymentMethodError" onchange="setPosPaymentMethod(this.value)"><option value="" ${paymentMethod ? "" : "selected"} disabled>Pilih pembayaran</option>${paymentMethods.map(method => `<option value="${method}" ${method === paymentMethod ? "selected" : ""}>${method === "Tunai" ? "Cash" : method}</option>`).join("")}</select>${paymentMethodInvalid ? `<small id="posPaymentMethodError" class="pos-field-error pos-payment-method-error" role="alert">Pilih metode pembayaran dulu.</small>` : ""}</div>
...
function posPaymentMethodValue() {
  ...
  const value = document.getElementById("paymentMethod")?.value || sessionStorage.getItem("pos_payment_method") || "";
...
function requirePosOrderTypeBeforePayment() {
  ...
  refreshPosFast({ cart: true, mobileBar: true, preserveScroll: true });
  toast("Pilih jenis pesanan dulu.");
  return "";
}
...
function requirePosPaymentMethod() {
  ...
  refreshPosFast({ cart: true, preserveScroll: true });
  toast("Pilih metode pembayaran dulu.");
  return "";
}

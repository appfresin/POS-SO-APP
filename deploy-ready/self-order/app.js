...
function updatePosSearch(input) {
  ...
  refreshPosOrRender({ preserveScroll: true, focusSearch: true, cursor });
}
...
function setOrderDiscountType(type) {
  ...
  refreshPosOrRender({ preserveScroll: true });
}
...
function pressPaymentKey(key) {
  ...
  if (key === "OK") {
    sessionStorage.setItem("pos_payment_slide", "quick");
    return refreshPosOrRender({ preserveScroll: true });
  }
  if (key === "C") current = "";
...
function setCategory(category) {
  ...
  refreshPosOrRender({ preserveScroll: false });
}
